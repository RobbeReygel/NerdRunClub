...
mkdir -p /etc/sentora
chown -R root:root /etc/sentora
unzip -oq sentora_core.zip -d /etc/sentora
mv "/etc/sentora/sentora-core-$SENTORA_CORE_VERSION" "/etc/sentora/panel"
rm sentora_core.zip
rm "/etc/sentora/panel/LICENSE.md" "/etc/sentora/panel/README.md" "/etc/sentora/panel/.gitignore"
rm -rf "/etc/sentora/_delete_me" "/etc/sentora/.gitignore"
...
mkdir -p /etc/sentora/configs
mkdir -p /etc/sentora/docs
chmod -R 777 /etc/sentora

mkdir -p $PANEL_DATA/backups
chmod -R 777 $PANEL_DATA/

# Links for compatibility with zpanel access
ln -s /etc/sentora /etc/zpanel
ln -s $PANEL_DATA /var/zpanel

#--- Prepare Sentora executables
chmod +x /etc/sentora/panel/bin/zppy
ln -s /etc/sentora/panel/bin/zppy /usr/bin/zppy

chmod +x /etc/sentora/panel/bin/setso
ln -s /etc/sentora/panel/bin/setso /usr/bin/setso

chmod +x /etc/sentora/panel/bin/setzadmin
ln -s /etc/sentora/panel/bin/setzadmin /usr/bin/setzadmin
...
cp -rf sentora-installers-$SENTORA_PRECONF_VERSION/preconf/* /etc/sentora/configs
rm sentora_preconfig*
rm -rf sentora-*

#--- Prepare zsudo
cc -o /etc/sentora/panel/bin/zsudo /etc/sentora/configs/bin/zsudo.c
sudo chown root /etc/sentora/panel/bin/zsudo
chmod +s /etc/sentora/panel/bin/zsudo
...
sed -i "s|YOUR_ROOT_MYSQL_PASSWORD|$mysqlpassword|" /etc/sentora/panel/cnf/db.php
mysql -u root -p"$mysqlpassword" < /etc/sentora/configs/sentora-install/sql/sentora_core.sql
...
mysql -u root -p"$mysqlpassword" < /etc/sentora/configs/sentora-install/sql/sentora_postfix.sql
...
#ln -s /etc/sentora/configs/postfix/transport /etc/postfix/transport
...
ln -s /etc/sentora/configs/postfix/master.cf /etc/postfix/master.cf
ln -s /etc/sentora/configs/postfix/main.cf /etc/postfix/main.cf
ln -s /etc/sentora/configs/postfix/vacation.pl /var/spool/vacation/vacation.pl

sed -i "s|!POSTFIX_PASSWORD!|$postfixpassword|" /etc/sentora/configs/postfix/*.cf
sed -i "s|!POSTFIX_PASSWORD!|$postfixpassword|" /etc/sentora/configs/postfix/vacation.conf
sed -i "s|!PANEL_FQDN!|$PANEL_FQDN|" /etc/sentora/configs/postfix/main.cf

sed -i "s|!USR_LIB!|$USR_LIB_PATH|" /etc/sentora/configs/postfix/master.cf
sed -i "s|!USR_LIB!|$USR_LIB_PATH|" /etc/sentora/configs/postfix/main.cf
sed -i "s|!SERVER_IP!|$PUBLIC_IP|" /etc/sentora/configs/postfix/main.cf

VMAIL_UID=$(id -u vmail)
MAIL_GID=$(sed -nr "s/^mail:x:([0-9]+):.*/\1/p" /etc/group)
sed -i "s|!POS_UID!|$VMAIL_UID|" /etc/sentora/configs/postfix/main.cf
sed -i "s|!POS_GID!|$MAIL_GID|" /etc/sentora/configs/postfix/main.cf

# remove unusued directives that issue warnings
sed -i '/virtual_mailbox_limit_maps/d' /etc/sentora/configs/postfix/main.cf
sed -i '/smtpd_bind_address/d' /etc/sentora/configs/postfix/master.cf
...
ln -s /etc/sentora/configs/dovecot2/globalfilter.sieve $PANEL_DATA/sieve/globalfilter.sieve

rm -rf /etc/dovecot/dovecot.conf
ln -s /etc/sentora/configs/dovecot2/dovecot.conf /etc/dovecot/dovecot.conf
sed -i "s|!POSTMASTER_EMAIL!|postmaster@$PANEL_FQDN|" /etc/sentora/configs/dovecot2/dovecot.conf
sed -i "s|!POSTFIX_PASSWORD!|$postfixpassword|" /etc/sentora/configs/dovecot2/dovecot-dict-quota.conf
sed -i "s|!POSTFIX_PASSWORD!|$postfixpassword|" /etc/sentora/configs/dovecot2/dovecot-mysql.conf
sed -i "s|!DOV_UID!|$VMAIL_UID|" /etc/sentora/configs/dovecot2/dovecot-mysql.conf
sed -i "s|!DOV_GID!|$MAIL_GID|" /etc/sentora/configs/dovecot2/dovecot-mysql.conf
...
if ! grep -q "Include /etc/sentora/configs/apache/httpd.conf" "$HTTP_CONF_PATH"; then
    echo "Include /etc/sentora/configs/apache/httpd.conf" >> "$HTTP_CONF_PATH";
fi
add_local_domain "$(hostname)"

if ! grep -q "apache ALL=NOPASSWD: /etc/sentora/panel/bin/zsudo" /etc/sudoers; then
    echo "apache ALL=NOPASSWD: /etc/sentora/panel/bin/zsudo" >> /etc/sudoers;
fi
...
if [[ "$OS" = "CentOs" ]]; then
    sed -i "s|DocumentRoot \"/var/www/html\"|DocumentRoot /etc/sentora/panel|" "$HTTP_CONF_PATH"
elif [[ "$OS" = "Ubuntu" ]]; then
...
if [[ ("$OS" = "CentOs" && "$VER" = "7") ||
    ...
    sed -i 's|Order deny,allow|Require all denied|I'  /etc/sentora/configs/apache/httpd.conf
    sed -i '/Deny from all/d' /etc/sentora/configs/apache/httpd.conf

    # Order allow,deny / Allow from all  ->  Require all granted
    sed -i 's|Order allow,deny|Require all granted|I' /etc/sentora/configs/apache/httpd-vhosts.conf
    sed -i '/Allow from all/d' /etc/sentora/configs/apache/httpd-vhosts.conf

    sed -i 's|Order allow,deny|Require all granted|I'  /etc/sentora/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php
    sed -i '/Allow from all/d' /etc/sentora/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php

    # Remove NameVirtualHost that is now without effect and generate warning
    sed -i '/NameVirtualHost/{N;d}' /etc/sentora/configs/apache/httpd-vhosts.conf
    sed -i '/# NameVirtualHost is/ {N;N;N;N;N;d}' /etc/sentora/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php

    # Options must have ALL (or none) +/- prefix, disable listing directories
    sed -i 's| FollowSymLinks [-]Indexes| +FollowSymLinks -Indexes|' /etc/sentora/panel/modules/apache_admin/hooks/OnDaemonRun.hook.php
fi
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    sed -i "s|nogroup|nobody|" /etc/sentora/configs/proftpd/proftpd-mysql.conf
elif [[ "$OS" = "Ubuntu" ]]; then
...
mysql -u root -p"$mysqlpassword" < /etc/sentora/configs/sentora-install/sql/sentora_proftpd.sql

# Create and configure mysql password for proftpd
proftpdpassword=$(passwordgen);
sed -i "s|!SQL_PASSWORD!|$proftpdpassword|" /etc/sentora/configs/proftpd/proftpd-mysql.conf
...
sed -i "s|!SQL_MIN_ID!|$HTTP_UID|" /etc/sentora/configs/proftpd/proftpd-mysql.conf

# Setup proftpd base file to call sentora config
rm -f "$FTP_CONF_PATH"
#touch "$FTP_CONF_PATH"
#echo "include /etc/sentora/configs/proftpd/proftpd-mysql.conf" >> "$FTP_CONF_PATH";
ln -s "/etc/sentora/configs/proftpd/proftpd-mysql.conf" "$FTP_CONF_PATH"
...
chmod -R 777 /etc/sentora/configs/bind/zones/
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    sed -i 's|bind/zones.rfc1918|named.rfc1912.zones|' /etc/sentora/configs/bind/named.conf
elif [[ "$OS" = "Ubuntu" ]]; then
    mkdir -p /var/named/dynamic
    touch /var/named/dynamic/managed-keys.bind
    chown -R bind:bind /var/named/
    chmod -R 777 /etc/sentora/configs/bind/etc
...
sed -i "s|!SERVER_IP!|$PUBLIC_IP|" /etc/sentora/configs/bind/named.conf

# Build key and conf files
rm -rf $BIND_FILES/named.conf $BIND_FILES/rndc.conf $BIND_FILES/rndc.key
rndc-confgen -a -r /dev/urandom
cat $BIND_FILES/rndc.key /etc/sentora/configs/bind/named.conf > $BIND_FILES/named.conf
cat $BIND_FILES/rndc.key /etc/sentora/configs/bind/rndc.conf > $BIND_FILES/rndc.conf
...
# sed -i "s|!USER!|$CRON_USER|" "/etc/sentora/configs/cron/zdaemon" #it screw update search!#
sed -i "s|!USER!|root|" "/etc/sentora/configs/cron/zdaemon"
cp "/etc/sentora/configs/cron/zdaemon" /etc/cron.d/zdaemon
...
chmod 644 /etc/sentora/configs/phpmyadmin/config.inc.php
sed -i "s|\$cfg\['blowfish_secret'\] \= 'SENTORA';|\$cfg\['blowfish_secret'\] \= '$phpmyadminsecret';|" /etc/sentora/configs/phpmyadmin/config.inc.php
ln -s /etc/sentora/configs/phpmyadmin/config.inc.php /etc/sentora/panel/etc/apps/phpmyadmin/config.inc.php
# Remove phpMyAdmin's setup folder in case it was left behind
rm -rf /etc/sentora/panel/etc/apps/phpmyadmin/setup
...
mysql -u root -p"$mysqlpassword" < /etc/sentora/configs/sentora-install/sql/sentora_roundcube.sql

# Create and configure mysql password for roundcube
roundcubepassword=$(passwordgen);
sed -i "s|!ROUNDCUBE_PASSWORD!|$roundcubepassword|" /etc/sentora/configs/roundcube/roundcube_config.inc.php
...
sed -i "s|!ROUNDCUBE_DESKEY!|$roundcube_des_key|" /etc/sentora/configs/roundcube/roundcube_config.inc.php

# Create and configure specials directories and rights
chown "$HTTP_USER:$HTTP_GROUP" "/etc/sentora/panel/etc/apps/webmail/temp"
...
ln -s /etc/sentora/configs/roundcube/roundcube_config.inc.php /etc/sentora/panel/etc/apps/webmail/config/config.inc.php
ln -s /etc/sentora/configs/roundcube/sieve_config.inc.php /etc/sentora/panel/etc/apps/webmail/plugins/managesieve/config.inc.php
...
/etc/sentora/panel/bin/setso --set sentora_domain "$PANEL_FQDN"
/etc/sentora/panel/bin/setso --set server_ip "$PUBLIC_IP"

# if not release, set beta version in database
if [[ $(echo "$SENTORA_CORE_VERSION" | sed  's|.*-\(beta\).*$|\1|') = "beta"  ]] ; then
    /etc/sentora/panel/bin/setso --set dbversion "$SENTORA_CORE_VERSION"
fi
